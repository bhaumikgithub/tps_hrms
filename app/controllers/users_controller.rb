...
class UsersController < ApplicationController
  ...
  def show
    ...
    @taken_leave = User.current_month_leave(@user, @start_date).to_f
    @free_leave = @user.free_leaves.find_by_leave_month(@start_date).to_f + 1
    @prev_leave_bal = @user.leave_reports.find_by_start_month(@start_date.at_beginning_of_month - 1.month).try(:current_leave_bal).to_f
