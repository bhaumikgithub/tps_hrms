class HomeController < ApplicationController
  ...
  def index
    ...
    @leaves = Leave.where('leave_date >= ? and leave_date <= ?', Date.today, Date.today + 7.days).where(status: "approved").order('leave_date ASC')
    ...
    @employees_leaves = Leave.joins(user: :user_designations).where(status: "approved").where('leave_date >= ? and leave_date <= ? and user_designations.mentor = ? and is_current = true', Date.today, Date.today + 14.days, current_user.id.to_s)
